...
function GamesTable(props) {
    ...
    useEffect(() => {
        if (filters.organizationId){
            dispatch(Actions.getGames({ filters, order, page, rowsPerPage }));
        }            
    }, [dispatch, filters, order, page, rowsPerPage, onGameMngGameModifiedData.data]);
    ...
    function handleRequestRemove(event, property) {
        dispatch(Actions.removeGames(selected, { filters, order, page, rowsPerPage }));
    }
    ...
    function handleChangePage(event, page) {
        dispatch(Actions.setGamesPage(page));
    }

    function handleChangeRowsPerPage(event) {
        dispatch(Actions.setGamesRowsPerPage(event.target.value));
    }
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <TablePagination
                ...
                labelRowsPerPage={T.translate("games.rows_per_page")}
                labelDisplayedRows={({ from, to, count }) => `${from}-${to === -1 ? count : to} ${T.translate("games.of")} ${count}`}
            />
        </div>
    );
}
