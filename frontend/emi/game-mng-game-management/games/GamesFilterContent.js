...
function TodoSidebarContent(props) {
    ...
    const { filters: { active: activeChecked } } = useSelector(({ GameManagement }) => GameManagement.games);
    const T = new MDText(i18n.get(user.locale));


    function handleActiveChange(evt) {
        if (activeChecked === null) {
            dispatch(Actions.setGamesFilterActive(true));
        } else if (activeChecked) {
            dispatch(Actions.setGamesFilterActive(false));
        } else {
            dispatch(Actions.setGamesFilterActive(null));
        }
    }
                ...
                <div className="p-24">
                    <FormGroup row>
                        <FormControlLabel
                            ...
                            label={T.translate("games.filters.active")}
                        />
                    </FormGroup>
                </div>
